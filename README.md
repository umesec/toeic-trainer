...
### 📝 文法・語彙クイズ
- TOEIC Part 5 形式の4択問題 **60問**（品詞／時制／前置詞／語彙）
- 回答直後に解説と訳を表示
- 間違えた問題は記録され、次回セッションで優先的に再出題

### 🗣️ Part 3/4 会話・トーク
- 会話（Part 3）×3セット + トーク（Part 4）×3セット、各3問の本番形式（計18問）
- 会話は話者ごとに声の高さを変えて逐次読み上げ。答え合わせでスクリプト全文と訳を確認できる

### 📄 Part 6/7 長文問題
- Part 6 長文穴埋め: 3文書×4空所（文挿入問題を含む）
- Part 7 長文読解: メール／広告／告知／記事の4文書×3問（NOT問題・言い換え問題を含む）
- 全問回答後に全文訳を表示できる
...
### 🎙️ Part 2 応答問題
- 本番の Part 2 形式 **25問**。質問と応答 (A)(B)(C) を音声のみで聞き、最も適切な応答を選ぶ
- 回答後に質問文・訳・「引っかけの型」の解説を表示（Where→場所、選択疑問文にYes/Noは不可、など）

### ⏱ 模試・実力測定（推定スコア）
- **ミニ模試**: 10分・15問（Part 2×5 + Part 5×10）。スキマ時間の腕試し用
- **実力測定モード**: 30分・47問（L: Part 2×10 + Part 3/4×4セット ／ R: Part 5×15 + Part 6×1文書 + Part 7×2文書）
...
- 実力測定モードの推定スコアは47問（本番は200問）による簡易推定で、**±50〜70点程度のブレがある目安**です。TTS音声・自作問題である点も含め、公式スコアを保証するものではありません
